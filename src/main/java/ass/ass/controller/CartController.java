...
public class CartController {
    ...
    @PostMapping("/add")
    public String addToCart(@RequestParam(required = false) String username, @RequestParam int productId,
        ...
        if (accounts == null && (username == null || username.isEmpty())) {
            // Nếu không có username từ session và không có username từ request, có thể xử
            // lý theo kiểu guest
            username = "guest"; // hoặc để null nếu bạn muốn xử lý giỏ hàng theo cách khác cho người dùng không
                                // đăng nhập
        ...
        try {
            // Gọi dịch vụ thêm sản phẩm vào giỏ hàng, không cần phải kiểm tra người dùng đã
            // đăng nhập hay chưa
